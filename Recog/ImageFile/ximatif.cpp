...
////////////////////////////////////////////////////////////////////////////////
CImageFileTif::CImageFileTif()//: CxImage(CXIMAGE_FORMAT_TIF) {}
{
	m_tif1=m_tif2=NULL; 
	m_multipage=false; 
	m_pages=0;
...
CImageFileTif::~CImageFileTif()
{
	if (m_tif1) TIFFClose(m_tif1);
...
bool CImageFileTif::Decode(int niPageIndex)
{
	...
	TIFF* m_tif = m_tif1;//_TIFFOpenEx(hFile, "rb");

	UINT height=0;
	UINT width=0;
	uint16 bitspersample=1;
	uint16 samplesperpixel=1;
	UINT rowsperstrip=(UINT)-1;
...
////////////////////////////////////////////////////////////////////////////////
bool CImageFileTif::Encode(CxFile * hFile, bool bAppend)
{
  cx_try
  {
...
bool CImageFileTif::Encode(CxFile * hFile, CxImage ** pImages, int pagecount)
{
  cx_try
  {
...
////////////////////////////////////////////////////////////////////////////////
bool CImageFileTif::EncodeBody(TIFF *m_tif, bool multipage, int page, int pagecount)
{
	UINT height=head.biHeight;
	UINT width=head.biWidth;
...
//	pitch = (uint16)CalculatePitch(line);
	...
	UINT rowsperstrip = TIFFDefaultStripSize(m_tif, (UINT) -1);  //<REC> gives better compression
	TIFFSetField(m_tif, TIFFTAG_ROWSPERSTRIP, rowsperstrip);
...
////////////////////////////////////////////////////////////////////////////////
void CImageFileTif::TileToStrip(BYTE* out, BYTE* in,	UINT rows, UINT cols, int outskew, int inskew)
{
	while (rows-- > 0) {
		UINT j = cols;
		while (j-- > 0)
...
////////////////////////////////////////////////////////////////////////////////
/*TIFF* CImageFileTif::TIFFOpenEx(CxFile * hFile)
{
...
////////////////////////////////////////////////////////////////////////////////
#include  <io.h>
bool CImageFileTif::ReadImageFile(FILE* fp, BYTE* lpExterRawBitsBuff/* = NULL*/, UINT uiBitsBuffSize /*= 0*/)
{
	if (fp==NULL)
		return false;
	int hFile=(int)_get_osfhandle(fp->_file);
	if(m_tif1!=NULL)
		TIFFCloseEx(m_tif1);
	if(m_tif1=TIFFFdOpen(hFile,"","rb"))//defined in tif_win32.c
		info.nNumFrames = m_pages = TIFFNumberOfDirectories(m_tif1);
	m_multipage=m_pages>1;
...
bool CImageFileTif::ReadImageFile(const char* szTiffFileName)
{
	if(m_tif1!=NULL)
		TIFFCloseEx(m_tif1);
	if(m_tif1=TIFFOpen(szTiffFileName,"rb"))//defined in tif_win32.c
		info.nNumFrames = m_pages = TIFFNumberOfDirectories(m_tif1);
	m_multipage=m_pages>1;
	return m_tif1!=NULL;
}
void CImageFileTif::TIFFCloseEx(TIFF* tif)
{
	if (tif) TIFFClose(tif);
}
////////////////////////////////////////////////////////////////////////////////
void CImageFileTif::MoveBits( BYTE* dest, BYTE* from, int count, int bpp )
{	int offbits = 0;
	uint16 w;
	UINT d;
	if (bpp <= 8) {
...
////////////////////////////////////////////////////////////////////////////////
void CImageFileTif::MoveBitsPal( BYTE* dest, BYTE*from, int count, int bpp, RGBQUAD* pal )
{	int offbits = 0;
	UINT d;
	uint16 palidx;
	while (count-- > 0) {
